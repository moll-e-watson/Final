...
-   Answer appears below operation in code chunk
:::
...
-   The value of a appears in the environment pane when you run the chunk

-   The value appears below the code chunk
:::
...
-   ^ is a power operator. a^2 calculates value in a to the power of 2
:::
...
-   sum() adds together all the arguments passed within it's braces. In this case a = 9, x = 6, y = 7, so 9+6+7 = 22
:::
...
-   Clicking broom icon emptied environment pane
:::
...
-   A loading circle popped up in my environment pane (is that supposed to happen? no idea.) ... It won't go away :(
...
-   It provides a new vector with each value increased by 2
:::
...
-   In the Environment pane, music appears, with chr[1:3] (chr describes its data type - character) and values "bluegrass", "funk", and "folk"
:::
